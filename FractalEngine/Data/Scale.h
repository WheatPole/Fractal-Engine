...
inline std::tuple<float, float> FindMiddle2(std::vector<std::tuple<float, float, float, float, int>>& selected) {
...
inline std::tuple<float, float, float, float> FindEdges(std::vector<std::tuple<float, float, float, float, int>>& selected) {
	float leftmostpoint = 99.0f;
	float lowermostpoint = 99.0f;
	float uppermostpoint = -99.0f;
	float rightmostpoint = -99.0f;
...
void ScaleAll(std::vector<std::tuple<float, float, float, float, int>>& selected, float xchange, float ychange, std::tuple<float, float>& middle) {
	auto edges = FindEdges(selected);
	auto newEdges = edges;
	std::get<0>(newEdges) += xchange;
	std::get<1>(newEdges) -= xchange;
