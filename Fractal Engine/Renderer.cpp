...
void Renderer::Rect(Renderer rend, Shader shader, float x1, float y1, float x2, float y2, bool fill) const {
	if (!fill) {
		float debug[] = {
		x1, y1,
		x2, y1,
		...
		vbl.Push<float>(2);
		VertexBuffer vb(debug, 4 * 2 * sizeof(float));
		IndexBuffer ib(indices, 6, 0);
		va.Add(vb, vbl);
		rend.Draw(va, ib, shader, GL_TRIANGLES);
	}
}
